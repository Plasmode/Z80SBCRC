...
;**************************************************************
;
;;; the jump table start from 0f200h
;;; overlap with jump table of cbiosZZ which also starts from 0f200h
...
;******************   E N D   O F   C P / M   *****************
;*
; 2/17/18 pass console input unfiltered
; CBIOS from CPM2 alteration guide, appendix C
; skeletal cbios for first level of cp/m 2.0 
msize 	equ 62	;cp/m version memory size in kilo
; "bias" is address offset from 3400h for memory system other than 16k
...
;;; Z80SBCRC CPLD serial port
RxData  	equ 0F9h        	; on-chip UART receive register
TxData  	equ 0F9h        	; on-chip UART transmit register
RxStat  	equ 0F8h        	; on-chip UART transmitter status/control register
TxStat  	equ 0F8h       	; 0n-chip UART receiver status/control register
;;; CF interface registers
; protoCF with base address at 0xC0
...
; fixed data tables for 4-drive standard ibm-compatible 8" disks
...
BOOT:
; initialization
	ld sp,100h	;;; stack is at 100h
;	xor a		;;; zero the accumulator
;	ld (track+1),a	;;; clear the MSB of track.  track is only 8-bit value
	...
	xor a		;zero in the accumulator
	ld (iobyte),a	;clear the iobyte
	ld (cdisk),a	;select disk zero
	jp gocpm		;initialize and go to cp/m
WBOOT:
; simplest case is to read the disk until all sector...
; CP/M is stored in LA 0x80-0x92.  The CCP & BDOS is from LA 0x80-0x91.  Warm boot will load
;  data from LA 0x80-0x91 
...
;	ld (track+1),a	; clear the MSB of track.  track is only 8-bit value
	...
	ld b,nsects	;reg B counts # of sectors to ...
;	ld c,0		;reg C has the current track
	ld de,200h		;start with sector 0x200 which is LA0x80
; note that we begin by reading track 0 sector 0x200, the beginning of CPM
...
; load one more sector
	...
	ld b,d
	ld c,e		;get sector address to reg BC
	call SETSEC	;set sector address from reg
...
;drive set to 0, track set, sector set, dma address
	call READ
	cp 0		;any errors?
	jp nz,WBOOT	;retry the entire boot if an error
; no error, move to next sector
	pop hl		;recall dma address
	ld de,128		;dma=dma+128
	add hl,de		;new dma address is in h,l
	pop de		;recall sector address
	pop bc		;recall number of sector remaining
	dec b		;sectors=sectors-1
	jp z,gocpm	; transfer to cp/m if all have been read
;more sector remain to load
	inc de
	jp load1		; no need to check next track
...
gocpm:
	...
	call SETDMA
;z80	ei		;enable the interrupt system
...
CONST:
	in a,(RxStat)	;;; read SerialCPLD receiver status
...
; console char into reg A
	in a,(RxStat)	;;; read Serial CPLD receiver status
	and 2		; data available?
	jp z,CONIN
...
; read char into reg A from reader device
	ld a,1ah		;enter end of file for now
	and 7fh		;remember to strip parity bit
	ret
; i/o drivers for the disk follow
; for now, we will simple store the parameters away in the
; read and write subroutines
HOME:
;move to the track 00 position of current drive
;translate this call into a settrk call with param
;;; track is only 8-bit.  track +1 is not changed
	ld bc,0		;select track 0
	call SETTRK
	ret		;we will move to 00 on first read
SELDSK
; select disk given by register C
;;; Maybe a work around to track limited to 8-bit:
;;;  track+1 denotes disk number for disk bigger than 32meg
...
;;; track is only 8-bit value.  track+1 is not changed
;	ld a,c
;	ld (track),a
	ld (track),bc
	ret
...
;;; no translate	ex de,hl		;hl=.trans
;;; no translate	add hl,bc		;hl=.trans(sector)
;;; no translate	ld l,(hl)		;l=trans(sector)
;;; no translate	ld h,0		;hl=trans(sector)
	ld h,b		;;; no translation
	ld l,c		;;; copy bc to hl
;	ld (sector),hl	; save value to sector <-- redundant, but accidentally
;			; make the whole thing worked before 16-bit track 
	ret		;with value in hl
SETDMA:
; set dma address given by register B and C
	ld l,c		;low order address
	ld h,b		; high order address
	ld (dmaad),hl	;save the address
	ret
READ:
;perform read operation (usual this is similar ...
;so we will allow space to set up read command...
	push hl		; save regs
	push de
	push bc
;Z80	ld c,1		;;8 assuming drive A (RAMdisk that starts at 0x10000 in RAM)
;Z80	ld a,(diskno)	; check if RAMdisk	
;Z80	cp 0		;;8 check RAMdisk A
;Z80	jp z,RAMDISKr
;Z80	ld c,4		;;8 not drive A, assume it is drive B (RAMdisk starts at 0x40000)
;Z80	cp 1		;;8 check RAMdisk B
;Z80	jp z,RAMDISKr
;;8	push hl		;save reg
;;8	push de
;;8	push bc
...
; assuming Little Endian storage
	ld a,(sector+1)	; get MSB sector address into reg B
	ld b,a
	ld a,(sector)	; get LSB sector address into reg A
	srl b		; 16-bit shift one bit
	rra
	srl b		; 16-bit shift another bit
	rra		; reg A has the 512-byte sector value
	ld b,a		; save it in b, need to use if in READnew
	ld hl,CFLA07	; CF LA LSB
	cp (hl)
	jp nz,getNEW
	ld a,(track)	; LSB track address
	inc hl		; points to CFLA815
	cp (hl)		; compare to CF LA 8-15
	jp nz,getNEW
	ld a,(track+1)	; MSB track address
	inc hl		; points to CFLA1623
	cp (hl)		; compare to CF LA 16-23
	jp z,move2DMA
getNEW:
	call READnew	; fetch new 512byte data from CF
; get the 128-byte data in the buffer to DMA address
move2DMA:
	ld de,(dmaad)	; get DMA address
	ld bc,128		; set up 128 byte memory to memory copy
	ld hl,CFsecdat0	; assume first 128 byte of buffer
	ld a,(sector)	; get LSB sector address
	and 3		; mask off hig bits
	jp z,movesec	; if zero then data is in first 128 bytes of buffer
	cp 1
	jp z,movesec1	; if 1 then data is in 2nd 128-byte block of buffer
	cp 2
	jp z,movesec2	; if 2 then data is in 3rd 128-byte block of buffer
	ld hl,CFsecdat3	; must be in last 128 bytes of buffer
	jp movesec
movesec1:
	ld hl,CFsecdat1
	jp movesec
movesec2:
	ld hl,CFsecdat2
movesec:
	ldir		; copy existing 128 bytes of data to DMA address
...
;;8	ld a,0E0h		; set Logical Address addressing mode
;;8	out (CF2427),a
...
;;8	in a,(CFstat)	; outjmp bug fix
	...
	ld b,0h		;;8
	inir		;;8
;;8	db 0edh,92h	; op code for inirw input word and increment
;	inirw
;;8	in a,(CFstat)	;outjmp bug fix
	ret
readdrq:
...
WRITE:
;1. pre-read into buffer
;2. merge with the 128 byte pointed by dma
;3. write out data
	push hl		;save reg
	push de
	push bc
...
; assuming Little Endian storage
	...
	srl b		; 16-bit shift another bit
	rra		; reg A has the 512-byte sector value
	ld b,a		; save it in b, need to use if in preREAD
	ld hl,CFLA07	; CF LA LSB
	cp (hl)
	jp nz,preREAD
	ld a,(track)	; LSB track address
...
DMA2BUF:
	...
	ld bc,128		; set up 128 byte memory to memory copy
	ld de,CFsecdat0	; assume first 128 byte of buffer
	ld a,(sector)	; get LSB sector address
	and 3		; mask off hig bits
	jp z,DMA2sec	; if zero then data is in first 128 bytes of buffer
	cp 1
	jp z,DMA2sec1	; if 1 then data is in 2nd 128-byte block of buffer
	cp 2
	jp z,DMA2sec2	; if 2 then data is in 3rd 128-byte block of buffer
	ld de,CFsecdat3	; must be in last 128 bytes of buffer
	jp DMA2sec
DMA2sec1:
	ld de,CFsecdat1
	jp DMA2sec
...
; no need to setup CFLA07, CFLA815, CFLA1623.  They already point to the correct LA
	...
	ld b,0h		;;8
	otir		;;8
;;8	db 0edh,93h	; op code for otirw, output word and increment
;	otirw
;;8	in a,(CFstat)	;outjmp bug fix
;	nop		;outjmp bug fix
;	nop
...
;	nop		;change program location
;	nop		; number of nop matters.  one more can cause program to hang
;	nop		; too few nop cause 'dir' command to print empty directory
;	nop
readbsy:
...
;*********************diagnostic uses ********** remove when done ***********
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
; wait UART transmit done before return
COUTdone:
...
signon:	
	...
	db "CP/M 2.2 for Z80SBCRC", 13,10
        	db "10/27/18 external CF disk",13,10,0 

scratch	ds 1
	org 0f800h	; fix location for BIOS variables
;the remainder of the cbios is reserved uninitializ...
...
CFsecdat0	ds 128		;;;CF sector data, 512 bytes
CFsecdat1 ds 128
CFsecdat2 ds 128
CFsecdat3	ds 128
